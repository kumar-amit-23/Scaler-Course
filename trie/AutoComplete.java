...
public class AutoComplete {
	...
	public static void main(String[] args) {
		...
		while (test_case-- > 0) {
			...
			// insert the words and weights into the TRIE
			for (int i = 0; i < n; i++) {
				insert(root, words[i], weights[i]);
			}

			String[] pre = new String[m];
			for (int i = 0; i < m; i++) {
				pre[i] = scn.next();
				ArrayList<Pair> pairList = search(root, pre[i]);

				if (!pairList.isEmpty()) {
					for (int j = 0; j < 5 && j < pairList.size(); j++) {
						System.out.print(pairList.get(j).str + " ");
					}
				} else {
					System.out.print(-1 + " ");
				}
	...
	public static ArrayList<Pair> search(TrieNode root, String pre) {
		...
		ArrayList<Pair> pairlist = curr.list;

		Collections.sort(pairlist, new sortByWeight());
		return pairlist;

	}

	static class sortByWeight implements Comparator<Pair> {
		public int compare(Pair a, Pair b) {
			return b.weight - a.weight; // sort in decreasing order.
		}
	}
}
