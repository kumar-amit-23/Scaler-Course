...
public class MapSumPairs {
	class TrieNode{
        ...
        HashSet<String> strArr;

        TrieNode(){
            this.value = 0;
            this.map = new HashMap<>();
            this.strArr = new HashSet<>();
        }
    }
    public ArrayList<Integer> solve(ArrayList<String> A, ArrayList<Integer> B) {
        ...
        for(int i = 0; i<A.size(); i++){
            if(B.get(i) != -1){
                ...
                int value = findWords(root, A.get(i));
                ans.add(value);
            }
        }
    ...
    public void insert(TrieNode root, String word, int value){
        TrieNode curr = root;
        ...
        for(int i = 0; i<word.length(); i++){
            ...
            curr.strArr.add(word);
        }

        curr.value = value;
    }

    public int findWords(TrieNode root, String pre){
        TrieNode curr = root;
        ...
        HashSet<String> words = curr.strArr;
        int sum = 0;
        for(String str : words){
            sum += findValuesOfWords(root, str);
        }

        return sum;
    }

    public int findValuesOfWords(TrieNode root, String word){
        TrieNode curr = root;
